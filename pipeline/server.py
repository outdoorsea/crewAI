...
from config.env_config import env_config, EnvConfig
...
async def _handle_chat_completions(request: dict):
    ...
    try:
        ...
        # Process through pipeline (fix: use correct parameter signature)
        response = pipeline.pipe(body=request)
...
async def crewai_pipeline_chat(request: Request):
...
async def chat_completions(request: Request):
    """Standard chat completions endpoint for OpenWebUI compatibility"""
    body = await request.json()
    return await _handle_chat_completions(body)
...
@app.post("/{pipeline_id}/chat/completions")
async def pipeline_chat_completions(pipeline_id: str, request: Request):
    """Handle chat completions for specific pipeline"""
    if pipeline_id == pipeline.id:
        body = await request.json()
        return await _handle_chat_completions(body)
    else:
...
def main():
    ...
    logger.info("📋 Add this URL to OpenWebUI Pipelines: http://localhost:9091")
    ...
    uvicorn.run(
        app,
        host=config.get_pipeline_host(),
        port=config.get_pipeline_port(),
        log_level="info"
    )
