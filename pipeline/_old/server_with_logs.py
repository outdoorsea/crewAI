    ...
    def pipe(
        ...
        try:
            ...
            if myndy_pipeline and hasattr(myndy_pipeline, 'pipe'):
                ...
                # Format body for the main myndy pipeline
                formatted_body = {
                    "messages": messages,
                    "model": model_id,
                    **body  # Include any additional body parameters
                }
                
                response = myndy_pipeline.pipe(body=formatted_body)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--pipeline":
        ...
        async def _handle_chat_completions(request: dict):
            """Shared chat completion handling logic"""
            try:
        ...
        async def chat_completions_main(request: dict):
            """Handle chat completion requests (main endpoint)"""
            return await _handle_chat_completions(request)

        @app.post("/v1/chat/completions")
        async def chat_completions_v1(request: dict):
            """Handle chat completion requests (v1 endpoint)"""
            return await _handle_chat_completions(request)
