...
class MyndyAPIClient:
    ...
    def _make_request_with_user_context(self, method: str, endpoint: str, user_context: Optional[Dict[str, Any]] = None, data: Optional[Dict] = None, params: Optional[Dict] = None) -> Dict[str, Any]:
        """
        Make HTTP request to Myndy-AI API with user context
...
class GetSelfProfileHTTPTool(BaseTool):
    """HTTP client tool for getting user profile via Myndy-AI API with user context support"""
    name: str = "get_self_profile"
    description: str = "Get the user's profile information via HTTP API. Automatically extracts user context from the current task execution."
    
    def _run(self, **kwargs) -> str:
        """Get self profile via HTTP API with user context"""
        try:
            ...
            # Call FastAPI endpoint with user context headers
            response = client._make_request_with_user_context("GET", "/api/v1/profile/self", user_context)
            ...
            result = {
                ...
                "note": "Profile retrieved via Myndy-AI FastAPI backend with user context"
            }
...
class UpdateSelfProfileHTTPTool(BaseTool):
    """HTTP client tool for updating user profile via Myndy-AI API with user context support"""
    name: str = "update_self_profile"
    description: str = "Update the user's profile information via HTTP API. Automatically extracts user context from the current task execution."
    
    def _run(self, name="", age="", email="", location="", occupation="", interests="", bio="", **kwargs) -> str:
        """Update self profile via HTTP API with user context"""
        try:
            # Extract user context from current execution
            user_context = _extract_user_context_from_task()
            ...
            # Call FastAPI endpoint with user context
            response = client._make_request_with_user_context("POST", "/api/v1/profile/self/update", user_context, data=profile_data)
