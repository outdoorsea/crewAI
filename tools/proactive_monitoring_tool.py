...
class MyndyStatusAPIClient:
    """HTTP client for myndy-ai status and memory API endpoints"""
    ...
    def __init__(self, base_url: str = "http://localhost:8000"):
        ...
        self.session.headers.update({
            ...
            "User-Agent": "CrewAI-ProactiveMonitoring/1.0"
        })
    ...
    def update_status(self, status_data: Dict[str, Any]) -> Optional[Dict]:
        """Update user status via API"""
        return self._make_request("POST", "/status/update", status_data)
    
    def get_status_history(self, limit: int = 10) -> Optional[Dict]:
        """Get status history via API"""
        return self._make_request("GET", f"/status/history?limit={limit}")
    
    def create_location_update(self, location_data: Dict[str, Any]) -> Optional[Dict]:
        """Create location update via API"""
        return self._make_request("POST", "/location/update", location_data)
    
    def get_health_metrics(self, date_range: Optional[str] = None) -> Optional[Dict]:
        """Get health metrics via API"""
        params = {"date_range": date_range} if date_range else {}
        return self._make_request("GET", "/health/metrics", params)

# Initialize API client
status_api_client = MyndyStatusAPIClient()
...
memory_manager = status_api_client
